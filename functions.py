...
def get_data(date_start, date_end):
    # filename to get meter usage data from
    filename = "meterusage.csv"
    # start date from user
    start = datetime.strptime(date_start, '%Y-%m-%d')
    # end date from user
    end = datetime.strptime(date_end, '%Y-%m-%d')
    # increment end date by 1 day to make user's choice inclusive
    end += timedelta(days=1, minutes=-15)
    # initialize empty dictionayr to return
    usage_dict = {}
    ...
    with open(filename, 'r') as meterusage_file:
        ...
        for row in meterusage_reader:
            ...
            # if date falls within range of users request
            if datetime_obj >= start and datetime_obj <= end:
                time = datetime_obj.strftime("%Y-%m-%d %H:%M:%S")
                # add datetime and meter usage value to dict
                usage_dict[time] = row[1]
    # if users requested dates dont exist, return a 404 error
    if not usage_dict:
        return str({}), 404
    # return dict as string
    return usage_dict

# print(get_data('2019-01-02', '2019-01-03'))
